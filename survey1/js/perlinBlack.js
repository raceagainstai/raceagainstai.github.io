// Constants
const AMOUNT_RINGS = 70;

const RADIUS_MAX = 1.35;
const RADIUS_MIN = 0.8;

const WIDTH_MAX = 0.8 * Math.PI; // radians
...
class RingGeometry extends THREE.BufferGeometry {
  ...
  constructor(radius, horizontal, vertical, width) {
    ...
    const horizontalStep = 6 * Math.PI / (horizontal - 1);
    const verticalOffset = width / 1.7 * -1.5;
    const verticalStep = width / 3.6 / (vertical - 1);
...
class Application {
  ...
  _setupLights() {
    ...
    lights.ambientLight = new THREE.AmbientLight('#111111');
    lights.hemisphereLight = new THREE.HemisphereLight(0x333333, 0x565656, 0.1);
