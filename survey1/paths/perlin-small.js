...
class RingGeometry extends THREE.BufferGeometry {
  ...
  constructor(radius, horizontal, vertical, width) {
    ...
    const horizontalStep = 3.5 * Math.PI / (horizontal - 1);
    const verticalOffset = width / 1 * -1;
    const verticalStep = width / 4.5 / (vertical - 1);
...
class Application {
  ...
  _setupLights() {
    ...
    lights.ambientLight = new THREE.AmbientLight('#0048bc');
    lights.hemisphereLight = new THREE.HemisphereLight(0x0000FF, 0x0000FF, 0.1);
  ...
  _setSize() {
    ...
    const viewportHeight = window.innerHeight * 0.175;
  ...
  _animateRing(ring, index) {
    TweenLite.to(ring.rotation, 1.0, {
       y: -Math.PI,
       x: 1 * Math.PI,
    ...
    TweenLite.to(ring.rotation, 1.8, {
       x: 2 * Math.PI,
       ease: Power1.easeInOut,
       delay: index / 200 });
  }
  ...
  _animateRingReverse(ring, index) {
    TweenLite.to(ring.rotation, 2.0, {
       y: -0.5,
       x: 0.6,
       ease: Power1.easeInOut,
       delay: index / 80,
